...
public://以下定义的成员变量和成员函数都是公有的
    ...
    {
        auto v0 = (VertexPoseAndIntrinsics *) _vertices[0];//_vertice[0]的type是VertexPoseAndIntrinsics
        auto v1 = (VertexPoint *) _vertices[1];//_vertice[1]的type是VertexPoint 
...
void SolveBA(BALProblem &bal_problem) {
    ...
    for (int i = 0; i < bal_problem.num_observations(); ++i) {
        ...
        edge->setVertex(0, vertex_pose_intrinsics[bal_problem.camera_index()[i]]);//等等DEBUG看一下
        edge->setVertex(1, vertex_points[bal_problem.point_index()[i]]);
        //_vertice[0]的type是VertexPoseAndIntrinsics
        //_vertice[1]的type是VertexPoint 
        //定义里edge的两条边？
    ...
    optimizer.optimize(100);//设置迭代次数为40
